...
mod textures {
    ...
    fn load_atlases(mut registry: ResMut<TextureAtlasRegistry>, asset_server: Res<AssetServer>, mut texture_atlases: ResMut<Assets<TextureAtlas>>) {
        ...
        for (identifier, atlas) in raw_atlases {
            ...
            registry.entities.insert(identifier, atlas.create_atlas_loaded(&asset_server, &mut texture_atlases));
        }
    ...
    impl RawGameAtlas {
        fn create_atlas_loaded(&self, asset_server: &Res<AssetServer>, texture_atlases: &mut ResMut<Assets<TextureAtlas>>) -> GameAtlas {
        ...
        fn create_atlas_handle(&self, asset_server: &Res<AssetServer>, texture_atlases: &mut ResMut<Assets<TextureAtlas>>) -> (Handle<Image>, Handle<TextureAtlas>) {
            let image_handle = asset_server.load(&self.texture);
            let texture_atlas = TextureAtlas::from_grid(image_handle.clone(), Vec2::new(self.tile_size_x, self.tile_size_y), self.columns, self.rows, None, None);
